...
func testOrgsExists(t *testing.T) {
	...
	e, err := OrgExists(ctx, tx, o.OrgID)
	if err != nil {
...
func testOrgsFind(t *testing.T) {
	...
	orgFound, err := FindOrg(ctx, tx, o.OrgID)
	if err != nil {
		t.Error(err)
	}
...
func testOrgToManyRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := OrgSlice{&a}
	if err = a.L.LoadRoles(ctx, tx, false, (*[]*Org)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOrgToManyZones(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, zoneDBTypes, false, zoneColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, zoneDBTypes, false, zoneColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	slice := OrgSlice{&a}
	if err = a.L.LoadZones(ctx, tx, false, (*[]*Org)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Zones); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Zones = nil
	if err = a.L.LoadZones(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Zones); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testOrgToManyAddOpRoles(t *testing.T) {
	var err error
...
func testOrgToManyAddOpZones(t *testing.T) {
	...
	foreigners := []*Zone{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, zoneDBTypes, false, strmangle.SetComplement(zonePrimaryKeyColumns, zoneColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Zone{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddZones(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Zones[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Zones[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Zones().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	orgDBTypes = map[string]string{`OrgID`: `integer`, `Name`: `character varying`}
	_          = bytes.MinRead
)
