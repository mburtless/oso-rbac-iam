...
func testUserToManyRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"user_roles\" (\"user_id\", \"role_id\") values ($1, $2)", a.UserID, b.RoleID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_roles\" (\"user_id\", \"role_id\") values ($1, $2)", a.UserID, c.RoleID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadRoles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpRoles(t *testing.T) {
	var err error
...
func testUserToManySetOpRoles(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpRoles(t *testing.T) {
	var err error
...
func testUserToOneOrgUsingOrg(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrg(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Org == nil {
...
func testUserToOneSetOpOrgUsingOrg(t *testing.T) {
	var err error
...
func testUserToOneRemoveOpOrgUsingOrg(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`UserID`: `integer`, `Name`: `text`, `APIKey`: `text`, `OrgID`: `integer`}
	_           = bytes.MinRead
)
